...
- (IBAction)moveToFolder:sender {
    ...
    [[self popupWithSubmenu:menu] showWithSender:sender
                                        andTitle:@"Move to folder..."];
}
...
- (id)popupWithSubmenu:(NSMenu *)submenu {
    ...
    [[NSBundle bundleForClass:[SearchPopup class]] loadNibNamed:@"SearchPopup"
                                                          owner:sp
                                                topLevelObjects:nil];
...
- (NSMenuItem *)newMenuItemWithTitle:(NSString *)title
                              action:(SEL)action
                    andKeyEquivalent:(NSString *)keyEquivalent
                              inMenu:(NSMenu *)menu
                           withTitle:(NSString *)searchTitle
                              offset:(int)offset
// Taken from
// /System/Developer/Examples/EnterpriseObjects/AppKit/ModelerBundle/EOUtil.m
{
    ...
    for (iI = 0; iI < [items count]; iI++) {
        ...
        if ([[menuItem title] isEqualToString:searchTitle]) {
            return ([menu insertItemWithTitle:title
                                       action:action
                                keyEquivalent:keyEquivalent
                                      atIndex:iI + offset]);
        } else if ([[menuItem target] isKindOfClass:[NSMenu class]]) {
            menuItem = [self newMenuItemWithTitle:title
                                           action:action
                                 andKeyEquivalent:keyEquivalent
                                           inMenu:[menuItem target]
...
- (void)setContextMenu:(NSMenu *)menu {
    ...
    // create "Move to Folder..." menu item just below the existing "Move Again"
    // menuitem
    NSMenuItem *moveFolderItem =
        [self newMenuItemWithTitle:@"Move to Folder..."
    ...
    for (int iI = 0; iI < [items count]; iI++) {
        ...
        if ([menuItem hasSubmenu]) {
            if ([menuItem tag] == 105)
                submenuMove = menuItem;
            if ([menuItem tag] == 106)
                submenuCopy = menuItem;
        }
    }
}
...
- (void)setLastFolder:(NSString *)folder {
    ...
    [menuitemLastMove
        setTitle:[NSString stringWithFormat:@"Move selected messages to \"%@\"",
                                            folder]];
    [menuitemLastCopy setHidden:FALSE];
    [menuitemLastCopy
        setTitle:[NSString stringWithFormat:@"Copy selected messages to \"%@\"",
                                            folder]];
}
