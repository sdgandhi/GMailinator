...
- (void)showWithSender:sender andTitle:(NSString *)title {
    ...
    } else if ([submenu respondsToSelector:@selector
...
NSInteger compareMatch(id l_row, id r_row, void *query) {
    double l_rank =
        calculate_rank((NSString *)[(NSDictionary *)l_row objectForKey:@"path"],
                       (__bridge NSString *)query);

    double r_rank =
        calculate_rank((NSString *)[(NSDictionary *)r_row objectForKey:@"path"],
                       (__bridge NSString *)query);

    if (l_rank == r_rank)
        return NSOrderedSame;
    else
        return l_rank > r_rank ? NSOrderedAscending : NSOrderedDescending;
}
        ...
        if ([searchString length] == 0 || is_subset(searchString, path))
            [currentResults
                addObject:[NSDictionary dictionaryWithObjectsAndKeys:
                                            path, @"path",
                                            [folders objectForKey:path],
                                            @"menuItem", nil]];
    }

    // Sort the folders according to how well they match with the search string.
    [currentResults sortUsingFunction:compareMatch
                              context:(__bridge void *)(searchString)];

    selectedResult =
        [currentResults count] > 0 ? [currentResults objectAtIndex:0] : nil;
...
- (BOOL)control:(NSControl *)control
    ...
    if (commandSelector == @selector(insertNewline:)) {
        if (selectedResult != nil) {
            //			[parent setLastFolder: selectedResult
            // objectAtIndex:0]];
        ...
        if (selectedResult != nil) {
            ...
            [resultViewer selectRowIndexes:[NSIndexSet indexSetWithIndex:index]
